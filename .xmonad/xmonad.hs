...
myEmacs, myGUIFileManager, myBrowser, myTerminal, myMPDClient, myGUIMusicApp, myCalculator, myTUIFileManager, myTmux
  :: String
...
myTUIFileManager =
  myTerminal
    ...
    ++ ""
    ++ "$HOME/local/bin/lfub"
myTmux = myTerminal ++ " " ++ "-e" ++ " " ++ "tmux" ++ " " ++ "attach"
 ...
 where
  ...
  spawnTerminal =
    myTerminal
      ...
      ++ "--class"
      ++ " "
      ++ "scratchpadterminal,ScratchPadTerminal"
...
myKeys =
  ...
  [
    ...
    , ( "M-<KP_Subtract>"
      ...
      ) -- Arrow Key <M+C+NumPad+> To Decrease THe Focused Window Size By 10 At      All    Sides
    , ( "M-<KP_Add>"
      , sendMessage (IncreaseUp 10)
      >> sendMessage (IncreaseDown 10)
      >> sendMessage (IncreaseLeft 10)
      >> sendMessage (IncreaseRight 10)
      ) -- Arrow Key <M+C+NumPad+> To Increase THe Focused Window Size By 10 At      All    Sides
    ...
    , ( "<XF86Search>"
      , spawn
        "rofi -show drun -theme $HOME/.config/rofi/launcher/drun/launcher.rasi"
      ) -- Use "Fn+F2" To Launch Rofi
    ...
    , ( "M-d S-d"
      , spawn
        "rofi -show run -theme $HOME/.config/rofi/launcher/run/launcher.rasi"
      )   -- Launcher -Run
...
spirals =
  renamed [Replace "Spiral"] $ mySpacing 3 $ hiddenWindows $ spiral (6 / 7)

masterAndStack =
  renamed [Replace "MasterAndStack"] $ mySpacing 3 $ hiddenWindows tiled
...
myLogHook dbus = filterOutWsPP [scratchpadWorkspaceTag] $ def
  ...
  , ppOrder   = \(ws : l : t : _) ->
                  [ ws
                  , "  "
                  , "%{A1:$HOME/.xmonad/xmonadctl 25:}" ++ l ++ "%{A}"
                  , "  "
                  ]                           -- Xmonad-Log Output With Workspaces And Current Layout
  , ppSep     = ""
  }
...
myStartupHook = do
  ...
  spawnOnce
    "rclone mount --daemon Drive_arbabashruff: $HOME/Mount/arbabashruff@gmail.com/"                                                                                                                                         -- Mount Drive Account On Local Machine
  spawnOnce "volctl"                                                                                                                                                                                                                -- Pipewire Volume Manager In SysTray
  spawnOnce "lxsession"                                                                                                                                                                                                             -- Polkit
  spawnOnce
    "xrandr --output DP-1 --off --output HDMI-1 --mode 1920x1080 --pos 0x0 --rotate normal --output HDMI-2 --primary --mode 1920x1080 --rate 144.00 --pos 1920x0 --rotate normal --output HDMI-3 --off"                     -- Multi-Screen Xrandr
  ...
  spawnOnce
    "feh --bg-fill $HOME/.xmonad/wallpapers/wallpaper1.png --bg-fill $HOME/.xmonad/wallpapers/wallpaper2.png --no-fehbg"                                                                                                    -- Set Background Multi-Screen
